...
quiz.question_02 = function() {
  ...
  // ----------------------------------------
  return [{John:'prof'},{Adel:'not-prof'}];
};
...
quiz.question_03 = function() {
  ...
  // ----------------------------------------
  return [{name:'John', age:35},{name:'Adel',age:35}];
};
...
quiz.question_04 = function(foo, bar) {
  ...
  // ----------------------------------------
  return {function1:(a,b)=>{return (a+b)},function2:(c,d)=>{return (a/b)}} ;
};
...
quiz.question_05 = function(someObject) {
  ...
  // ----------------------------------------
  someObject.age = 'any value I want';
  return someObject;
};
...
quiz.question_06 = function(data, carName, model, doors, color) {
  ...
  var carPrice = 0;
  // TODO your code here
  data.cars[carName].forEach((current)=>{
    console.log(current)
    if (current.model === model && current.doors === doors){
      carPrice = current.price
    };
  });
...
quiz.question_07 = function(data) {
  ...
  var maxPricedCar = {};
  maxPricedCar.price =0
  // TODO your code here
  for(var arg in data.cars){
    data.cars[arg].forEach((car)=>{
